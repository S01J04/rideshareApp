...
interface DestinationProps {
  ...
  setToCoordinates: (coordinates: { lat: number, lng: number } | null) => void
  onNext: () => void
}

const Destination = ({ toLocation, setToLocation, setToCoordinates, onNext }: DestinationProps) => {
  ...
  useEffect(() => {
    console.log("Destination suggestions state updated:", suggestions);
  }, [suggestions]);
  ...
  const fetchSuggestions = useCallback(
    debounce(async (input: string) => {
      ...
      try {
        ...
        console.log("Fetching destination suggestions from API for:", input);
  ...
  const handleDestinationInput = (text: string) => {
    ...
    if (matchedLocation && text.length >= 3) {
      ...
      setSuggestions(commonLocations[matchedLocation]);
  ...
  const selectDestination = async (location: string) => {
    if (!location) return;
    ...
    try {
      ...
      if (coords) {
        ...
        if (location.toLowerCase().includes('karachi')) {
          setToCoordinates({ lat: 24.8607, lng: 67.0011 });
        } else if (location.toLowerCase().includes('lahore')) {
          setToCoordinates({ lat: 31.5204, lng: 74.3587 });
        } else if (location.toLowerCase().includes('islamabad')) {
          setToCoordinates({ lat: 33.6844, lng: 73.0479 });
        } else if (location.toLowerCase().includes('peshawar')) {
          setToCoordinates({ lat: 34.0151, lng: 71.5249 });
        } else if (location.toLowerCase().includes('risalpur')) {
          setToCoordinates({ lat: 34.0151, lng: 72.0249 });
        } else {
          // Use the coordinates from the console log as a fallback
          setToCoordinates({ lat: 34.2135224, lng: 72.2836333 });
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    if (!toLocation || toLocation.trim().length < 3) {
      Alert.alert("Invalid Location", "Please enter a valid destination.");
      return;
    }
    ...
    try {
      ...
      const response = await getCoordinates(toLocation);
      ...
      if (coords) {
        ...
        // For testing/demo purposes, use hardcoded coordinates based on location name
        if (toLocation.toLowerCase().includes('karachi')) {
          setToCoordinates({ lat: 24.8607, lng: 67.0011 });
        } else if (toLocation.toLowerCase().includes('lahore')) {
          setToCoordinates({ lat: 31.5204, lng: 74.3587 });
        } else if (toLocation.toLowerCase().includes('islamabad')) {
          setToCoordinates({ lat: 33.6844, lng: 73.0479 });
        } else if (toLocation.toLowerCase().includes('peshawar')) {
          setToCoordinates({ lat: 34.0151, lng: 71.5249 });
        } else if (toLocation.toLowerCase().includes('risalpur')) {
          setToCoordinates({ lat: 34.0151, lng: 72.0249 });
        } else {
          // Use the coordinates from the console log as a fallback
          setToCoordinates({ lat: 34.2135224, lng: 72.2836333 });
  ...
  return (
    <View style={styles.container}>
      ...
      {!isLoading && toLocation.length > 0 && toLocation.length <= 3 && suggestions.length === 0 && !error && (
        <Text style={styles.helperText}>
